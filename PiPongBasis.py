...
class Lotse:
    ...
    def Eval(self, data, pl, pr, po, pu):
        if data & pl > 0:
            self.Links = True
        if data & pr > 0:
            self.Rechts = True
    ...
    def __str__(self):
        return "(%s) %s %s %s %s" % self.Schritte \
            % "LI " if self.Links else "" \
            % "RE " if self.Rechts else "" \
            % "OB " if self.Oben else "" \
            % "UN " if self.Unten else ""
...
class SpielBasis(SampleBase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.Seriell = None
        self.FarbeSchwarz = self.GebeFarbe(0, 0, 0)
        ...
        self.FarbeWeiss = self.GebeFarbe(255, 255, 255)
        self.players = [] 
        self.SpielStart()

    def SeriellOeffnen(self):
        self.Seriell = serial.Serial("/dev/ttyACM0", 115200, timeout = 0)

    def LeereLotsen(self):
        return ( Lotse(), Lotse() )
    ...
    def SeriellLeseLotsen(self):
        ...
        while True:
            # hole neue Daten
            if not self.Seriell.inWaiting():
                break
            st = self.Seriell.readline().strip()
            if len(st) < 14:
                continue
            ...
            sw = int(st[0:4], 16)
            enc1 = int(st[5:9], 16)
            enc2 = int(st[10:14], 16)

            # auswerten
            l1.Eval(sw, 0x10, 0x04, 0x40, 0x20)
    ...
    def MaleLinie(self, x1, y1, x2, y2, farbe):
        """ Malt eine Linie zwischen den Koordinaten (x1,y1) und (x2,y2).
        Die Koordinaten sind 0-basiert. """

        # for y in range(y1,y2+1):
        #     graphics.DrawLine(self.double_buffer, x1, y, x2, y, farbe)
        self.DrawLine(self.double_buffer, x1, y1, x2, y2, farbe)
    ...
    def MaleSprite(self, x1, y1, sprite : PixelKarte):
        self.DrawSprite(self.double_buffer, x1, y1, x1+sprite.width-1, y1+sprite.height-1, sprite.RgbDaten())

    def LadeFont(self, fontPfad):
        """ Laedt einen Zeichsatz aus der angegebenen .bdf Datei """
        return self.LoadFont(fontPfad)

    def MaleText(self, font, x, y, farbe, text):
        """ Malt einen Text mit dem angegebenen Font und Farbe an Postion x,y """
        self.DrawText(self.double_buffer, font, x, y, farbe, text)
